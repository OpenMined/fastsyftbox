...
class SyftBoxSDK:
    ...
    async def syft_make_request(self, syft_url: str, options: dict | None = None):
        if options is None:
            options = {}
        from_email = options.get("headers", {}).get("x-syft-from", ANONYMOUS_EMAIL)
    ...
    async def send_request(self, request: SyftRequest):
        ...
        try:
            ...
            raise SyftError(
                f"Error {response.status_code}: {error_text}", "REQUEST_ERROR"
            )
        except Exception as error:
    ...
    async def _poll_request(self, poll_url, request_id):
        ...
        print("poll response", response.status_code, response.json())
